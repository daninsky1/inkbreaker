...
public:
    ...
    BezierHandle *add_point(const BezierHandle point);
    BezierHandle *add_point(Vec2f point, Vec2f head, Vec2f tail);
    BezierHandle *insert_point(int index, const BezierHandle point);
    BezierHandle *last_point() { return &bhandles[bhandles.size() - 1]; }
